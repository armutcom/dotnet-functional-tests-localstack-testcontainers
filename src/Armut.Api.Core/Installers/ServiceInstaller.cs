...
namespace Armut.Api.Core.Installers
{
    public class ServiceInstaller: IInstaller
    {
        public void Install(IServiceCollection services, IConfiguration configuration, IHostEnvironment hostEnvironment)
        {
            ...
            {
                bool isComponent = type.GetInterfaces().Contains(typeof(IComponent));
                ...
                Type componentInterfaceType = type.GetInterfaces().FirstOrDefault(interfaceType => interfaceType.GetInterfaces().Contains(typeof(IComponent)) && interfaceType.GetInterfaces().Length == 1);
